...
def append_word_embedding_gensim(vocab, w2v_file, embeddings_g, pre_trained_g):
    pre_trained = pre_trained_g
    n_words = len(vocab)
    dim = 300
    embeddings = embeddings_g

    model_wv = KeyedVectors.load_word2vec_format(w2v_file, binary=True)
    
    for word in vocab:
        if word not in pre_trained:
             if word in model_wv:
                 embeddings[vocab[word]] = model_wv[word]
                 pre_trained[word] = 1
    ...
    pickle.dump(oov_word_list, open('oov_glove_rob04_after_filter.p', 'wb'), protocol=2)
...
def load_word_embedding_gensim(vocab, w2v_file):
    ...
    oov_word_list = [w for w in vocab if w not in pre_trained]
    # print('oov word list example (30): ', oov_word_list[:30])
    # pickle.dump(oov_word_list, open('oov2.p', 'wb'), protocol=2)
