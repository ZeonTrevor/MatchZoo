...
class NN_client:
    ...
    def get_bg_docs_idf_ql(self, query, doc_len):
        data = dict()
        data["query"] = query
        data["doc_len"] = doc_len
        self.headers["Connection"] = "close"
        ...
        bg_doc_ql_inv_r_low = response["doc_ql_inv_r_low"]
        bg_doc_ql_std_low = response["doc_ql_std_low"]
        return bg_doc_idf, bg_doc_idf_ql_low, bg_doc_idf_ql_rand, bg_doc_ql_inv_r_low, bg_doc_ql_std_low

    def get_bg_query(self, query_len):
        data = dict()
        data["query_len"] = query_len
        self.headers["Connection"] = "close"
        response = requests.post(self.url + "get_bg_query", json=data, headers=self.headers).json()
        bg_query_idf_low_rand = response["query_idf_low_rand"]
        bg_query_rand = response["query_rand"]
        return bg_query_idf_low_rand, bg_query_rand
...
if __name__ == '__main__':
    ...
    # print(cli.get_doc_bm25_score(query, "FBIS3-10082"))

    # docs_text, scores = cli.get_bm25_matched_docs(query)
    # print("\n".join(docs_text[:10]))
    # print(scores[:10])

    # bg_doc_idf, bg_doc_idf_ql_low, bg_doc_idf_ql_rand = cli.get_bg_docs_idf_ql(query)
    # print(len(doc_vec))

    # print(doc_vec.count(106662))
    # print(doc_cont)
    # print(" ".join(doc_vec))
    # print(cli.transform_doc(" ".join(doc_vec)))
