...
class DSSM(BasicModel):
    ...
    def build(self):
        def matrix_mul(x):
            t1 = x
            t2 = K.variable(value=self.config['embed'])  # x[1]
            out = K.tf.matmul(t1, t2)
            return out

        query = Input(name='query', shape=(self.config['vocab_size'],))#, sparse=True)
        # query = Input(name='query', shape=(1,))
        ...
        def mlp_work(input_dim):
            ...
            if num_hidden_layers == 1:
                ...
                b = np.zeros(300)
                dense_layer1 = Dense(self.config['hidden_sizes'][0], activation='tanh', input_shape=(input_dim,), activity_regularizer=regularizers.l2(self.config['reg_rate']), weights=[self.config['embed'], b])
                seq.add(dense_layer1)
                print('added dense layer with pretrained triletter embeddings')
                ...
                for i in range(num_hidden_layers-2):
                    seq.add(Dense(self.config['hidden_sizes'][i+1], activation='tanh', activity_regularizer=regularizers.l2(self.config['reg_rate']), kernel_initializer='glorot_uniform'))
                    seq.add(Dropout(rate=self.config['dropout_rate']))
                seq.add(Dense(self.config['hidden_sizes'][num_hidden_layers-1], activity_regularizer=regularizers.l2(self.config['reg_rate']), kernel_initializer='glorot_uniform'))
                seq.add(Dropout(rate=self.config['dropout_rate']))
            return seq
        ...
        for layer_out in mlp.layers:
            print('[shape]: %s \n%s' % (str(layer_out.get_output_at(0).get_shape().as_list()), show_memory_use()))
        print('model summary')
        print(mlp.summary())
